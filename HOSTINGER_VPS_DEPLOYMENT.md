# Omingle deployment on Hostinger VPS
...
mkdir -p /var/www
cd /var/www
git clone https://github.com/YOUR_GITHUB_USERNAME/Omingle.git omingle
cd /var/www/omingle
```
...
- `CORS_ORIGINS=https://omingle.fun,https://www.omingle.fun`
...
```bash
cp deploy/nginx-omingle.fun.conf /etc/nginx/sites-available/omingle.fun
ln -s /etc/nginx/sites-available/omingle.fun /etc/nginx/sites-enabled/omingle.fun
nginx -t
...
```bash
certbot --nginx -d omingle.fun -d www.omingle.fun
```
...
- `HOSTINGER_APP_DIR` → `/var/www/omingle`
