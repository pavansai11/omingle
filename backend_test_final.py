#!/usr/bin/env python3
"""
Final corrected Omingle Backend Test Suite
All tests working properly with corrected event names
"""
...
class OmingleTesterCorrect:
    ...
    async def test_socketio_text_chat(self):
        """Test Socket.io text chat functionality with correct event names"""
        try:
            sio1 = socketio.AsyncClient(logger=False, engineio_logger=False)
            sio2 = socketio.AsyncClient(logger=False, engineio_logger=False)
            ...
            # CORRECTED: Use kebab-case event name 'receive-message' not 'receive_message'
            @sio2.on('receive-message')
            ...
            if client1_matched and client2_matched and room_id:
                ...
                message_data = {
                    'roomId': room_id,
                    'message': 'Hello from Spanish speaker!',
                    'fromLang': 'es'
                }
                ...
                if message_received and received_message_data:
                    if (received_message_data.get('text') == 'Hello from Spanish speaker!' and
    ...
    async def test_socketio_next_leave(self):
        """Test Socket.io next/leave functionality with correct event names"""
        try:
            sio1 = socketio.AsyncClient(logger=False, engineio_logger=False)
            sio2 = socketio.AsyncClient(logger=False, engineio_logger=False)
            ...
            # CORRECTED: Use kebab-case event name 'partner-left' not 'partner_left'
            @sio2.on('partner-left')
