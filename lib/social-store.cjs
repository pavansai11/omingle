...
async function getUserReputation(userId) {
  ...
  if (!collections) {
    const existingProfile = memory.profiles.get(userId) || {};
    const existingRep = memory.reputation.get(userId) || {};
    return {
      likesReceived: Number.isFinite(existingProfile.likesReceived) ? existingProfile.likesReceived : (Number.isFinite(existingRep.likesReceived) ? existingRep.likesReceived : 0),
      reportsReceived: Number.isFinite(existingProfile.reportsReceived) ? existingProfile.reportsReceived : (Number.isFinite(existingRep.reportsReceived) ? existingRep.reportsReceived : 0),
    };
  }

  const user = await collections.users.findOne(
    { $or: [{ userId }, { googleId: userId }] },
    { projection: { likesReceived: 1, reportsReceived: 1 } }
  );

  return {
    likesReceived: Number.isFinite(user?.likesReceived) ? Number(user.likesReceived) : 0,
    reportsReceived: Number.isFinite(user?.reportsReceived) ? Number(user.reportsReceived) : 0,
  };
}

async function incrementUserReputation(userId, increments = {}) {
  if (!userId) return { likesReceived: 0, reportsReceived: 0 };

  const likesDelta = Number(increments?.likesReceived || 0);
  const reportsDelta = Number(increments?.reportsReceived || 0);
  const collections = await getCollections();
  ...
  if (!collections) {
    ...
    const existingRep = memory.reputation.get(userId) || {};
    const next = {
      likesReceived: Math.max(0, Number(existingProfile.likesReceived ?? existingRep.likesReceived ?? 0) + likesDelta),
      reportsReceived: Math.max(0, Number(existingProfile.reportsReceived ?? existingRep.reportsReceived ?? 0) + reportsDelta),
      updatedAt: new Date(),
    };
    memory.reputation.set(userId, next);
    memory.profiles.set(userId, { ...existingProfile, ...next, userId, updatedAt: new Date(), createdAt: existingProfile.createdAt || new Date() });
    return next;
  }

  const incPayload = {};
  if (likesDelta) incPayload.likesReceived = likesDelta;
  if (reportsDelta) incPayload.reportsReceived = reportsDelta;
  if (!Object.keys(incPayload).length) {
  ...
  await collections.users.updateOne(
    ...
    {
      $inc: incPayload,
      $set: { updatedAt: new Date(), userId, googleId: userId },
      $setOnInsert: { createdAt: new Date() },
    },
    { upsert: true }
  );
...
async function recordMatchHistoryForUsers(userA, userB, meta = {}) {
  ...
  if (!collections) {
    ...
    const capForOwner = (ownerUserId) => {
      let kept = 0;
      const next = [];
      for (const item of memory.matchHistory) {
      ...
      memory.matchHistory = next;
    };
    capForOwner(a.userId);
    capForOwner(b.userId);
    return;
  }

  await collections.matchHistory.insertMany(records);

  async function pruneOwnerHistory(ownerUserId) {
    const staleItems = await collections.matchHistory
      .find({ ownerUserId }, { projection: { _id: 1 } })
      .sort({ connectedAt: -1, createdAt: -1, _id: -1 })
      .skip(10)
      .toArray();

    if (!staleItems.length) return;
    const staleIds = staleItems.map((item) => item._id).filter(Boolean);
    if (!staleIds.length) return;
    await collections.matchHistory.deleteMany({ _id: { $in: staleIds } });
  }

  await Promise.all([pruneOwnerHistory(a.userId), pruneOwnerHistory(b.userId)]);
}
