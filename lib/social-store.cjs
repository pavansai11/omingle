  ...
  try {
    if (!cache.promise) {
      ...
      cache.promise = client.connect().then((connectedClient) => {
        ...
        cache.db = connectedClient.db(process.env.DB_NAME || 'Omingle');
        return cache.db;
      });
    }
...
function normalizeProfile(profile) {
  ...
  return {
    ...
    name: profile.name || 'Omingle User',
    email: profile.email || '',
...
async function listHistory(userId) {
  ...
  if (!collections) {
    items = memory.matchHistory.filter((item) => item.ownerUserId === userId).slice(0, 50);
  } else {
    items = await collections.matchHistory.find({ ownerUserId: userId }).sort({ connectedAt: -1, createdAt: -1 }).limit(50).toArray();
  }
