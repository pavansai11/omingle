// ─── ADD THIS BLOCK inside the GET handler in route.js ───────────────────────
// after the existing `if (path === 'turn-credentials')` block:

/*
  if (path === 'stats') {
    // Serve last-known stats from Redis for homepage display
    try {
      const { getJson } = await import('@/lib/upstash-redis')
      const stats = await getJson('hippichat:stats').catch(() => null)
...
// ─── The full updated GET export (drop-in replacement) ────────────────────────
// This version adds the /api/stats endpoint.
// Copy this entire export to replace the existing GET in route.js.
...
function sanitizeLanguageSelection(language) {
  ...
  return { code, name: sanitizeString(language.name, 80) || code, flag: sanitizeString(language.flag, 12) || '🌐' };
}

function sanitizeLanguageSelections(languages) {
  if (!Array.isArray(languages)) return undefined;
  return languages.map((l) => sanitizeLanguageSelection(l)).filter(Boolean).slice(0, 5);
}
...
async function getAdEngagementForUser(userId) {
  ...
  return {
    ...
    pendingGate: pendingGate?.nonce ? { reason: pendingGate.reason || null, nonce: pendingGate.nonce, gateOpenedAt: pendingGate.gateOpenedAt || null, minEligibleAt: pendingGate.minEligibleAt || null, expiresAt: pendingGate.expiresAt || null } : null,
  };
}
...
async function setAdEngagementForUser(userId, engagement) {
  ...
  const db = await getDatabase();
  if (!db) { memoryAdEngagement.set(userId, sanitized); return sanitized; }
  const users = db.collection('users');
  await users.updateOne({ $or: [{ userId }, { googleId: userId }] }, { $set: { adEngagement: sanitized, updatedAt: new Date() } });
  return sanitized;
}
...
function normalizeLangCode(lang) {
  ...
  const aliasMap = { 'jp': 'ja', 'iw': 'he', 'zh-cn': 'zh-Hans', 'zh-tw': 'zh-Hant', 'zh-hk': 'zh-Hant', 'pt-br': 'pt-BR', 'pt-pt': 'pt-PT' };
...
async function getTurnCredentialPayload() {
  ...
  if (!tokenId) throw new Error('CLOUDFLARE_TURN_TOKEN_ID is not configured');
  if (!apiToken) throw new Error('CLOUDFLARE_TURN_API_TOKEN is not configured');
  const response = await fetch(
    `https://rtc.live.cloudflare.com/v1/turn/keys/${encodeURIComponent(tokenId)}/credentials/generate-ice-servers`,
    { method: 'POST', headers: { Authorization: `Bearer ${apiToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ ttl: ttlSeconds }) }
  );
  const data = await response.json().catch(() => null);
  const iceServers = Array.isArray(data?.iceServers) ? data.iceServers : Array.isArray(data?.result?.iceServers) ? data.result.iceServers : null;
  if (!response.ok) throw new Error(`Cloudflare TURN API request failed with status ${response.status}`);
  if (!iceServers?.length) throw new Error('Cloudflare TURN API returned no iceServers');
  return { iceServers, ttlSeconds, provider: 'cloudflare' };
}
...
export async function GET(request, { params }) {
  ...
  if (path === 'auth/session') {
    const sessionId = request.cookies.get(getSessionCookieName())?.value;
    const session = await getUserSession(sessionId);
    return NextResponse.json({ user: session?.user || null, authenticated: !!session?.user });
  }
  ...
  // ── NEW: stats endpoint for homepage online count display ──────────────────
  if (path === 'stats') {
    try {
      // Import redis lazily so it doesn't break if unavailable
      const redis = await import('@/lib/upstash-redis.cjs').then(m => m.default || m).catch(() => null);
      if (redis?.getJson) {
        const stats = await redis.getJson('hippichat:stats').catch(() => null);
        return NextResponse.json({ online: stats?.online ?? null, rooms: stats?.rooms ?? null });
      }
      return NextResponse.json({ online: null });
    } catch (e) {
      return NextResponse.json({ online: null });
...
export async function POST(request, { params }) {
  ...
  if (path === 'profile') {
    try {
      ...
      // customImage intentionally removed — Google photo is always used
  ...
  if (path === 'auth/google') {
    try {
      ...
      if (!safeCredential) return NextResponse.json({ error: 'Missing Google credential' }, { status: 400 });
      const expectedAudience = process.env.GOOGLE_CLIENT_ID || process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;
      if (!expectedAudience) return NextResponse.json({ error: 'Google sign-in is not configured' }, { status: 500 });
      ...
      if (verifyData.aud !== expectedAudience) return NextResponse.json({ error: 'Google token audience mismatch' }, { status: 401 });
      if (!ALLOWED_GOOGLE_ISSUERS.has(verifyData.iss)) return NextResponse.json({ error: 'Google token issuer mismatch' }, { status: 401 });
      if (verifyData.email_verified !== 'true') return NextResponse.json({ error: 'Google account email is not verified' }, { status: 401 });
      const user = await upsertGoogleUser(verifyData);
      const session = await createUserSession(user);
      const response = NextResponse.json({ user });
      response.cookies.set({ name: getSessionCookieName(), value: session.sessionId, httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', path: '/', expires: new Date(session.expiresAt) });
      return response;
    } catch (error) {
  ...
  if (path === 'auth/logout') {
    ...
    response.cookies.set({ name: getSessionCookieName(), value: '', httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', path: '/', expires: new Date(0) });
    return response;
  }
  ...
  if (path === 'ad-engagement') {
    ...
    if (action === 'open-gate') {
      ...
      if (!ALLOWED_GATE_REASONS.has(reason)) return NextResponse.json({ error: 'Unsupported gate reason' }, { status: 400 });
      const now = Date.now();
      const pendingGate = { reason, nonce: crypto.randomUUID(), gateOpenedAt: new Date(now).toISOString(), minEligibleAt: new Date(now + AD_GATE_MIN_MS).toISOString(), expiresAt: new Date(now + AD_GATE_MAX_MS).toISOString() };
      const saved = await setAdEngagementForUser(user.id, { skipCount: currentSkipCount, pendingGate });
      return NextResponse.json({ ok: true, pendingGate: saved.pendingGate, serverNow: new Date().toISOString() });
    }

    if (action === 'skip-attempt') {
      const nextSkipCount = currentSkipCount + 1;
      const shouldGate = nextSkipCount % 10 === 0;
      const saved = await setAdEngagementForUser(user.id, { skipCount: nextSkipCount, pendingGate: engagement?.pendingGate || null });
      return NextResponse.json({ skipCount: saved.skipCount, shouldGate });
    }
    ...
    if (action === 'complete-gate') {
      ...
      if (!pendingGate?.nonce) return NextResponse.json({ error: 'No pending gate' }, { status: 400 });
      if (pendingGate.nonce !== nonce || pendingGate.reason !== reason) return NextResponse.json({ error: 'Invalid gate token' }, { status: 403 });
      const now = Date.now();
      const expiresAt = new Date(pendingGate.expiresAt).getTime();
      if (Number.isFinite(expiresAt) && now > expiresAt) {
        const cleared = await setAdEngagementForUser(user.id, { skipCount: currentSkipCount, pendingGate: null });
      ...
      const saved = await setAdEngagementForUser(user.id, { skipCount: nextSkipCount, pendingGate: null });
      return NextResponse.json({ ok: true, reason, skipCount: saved.skipCount });
    }
  ...
  if (path === 'translate') {
    try {
      ...
      if (!safeText || !safeFrom || !safeTo) return NextResponse.json({ error: 'Missing fields: text, from, to required' }, { status: 400 });
      if (safeText.trim().length === 0) return NextResponse.json({ translatedText: '' });
      const sourceLang = normalizeLangCode(safeFrom);
      const targetLang = normalizeLangCode(safeTo);
      if (!sourceLang || !targetLang) return NextResponse.json({ error: 'Invalid language code(s)' }, { status: 400 });
      if (sourceLang === targetLang) return NextResponse.json({ translatedText: safeText });
      return NextResponse.json({ translatedText: safeText, provider: 'disabled-no-azure', fallback: true, sourceLang, targetLang });
    } catch (error) {
